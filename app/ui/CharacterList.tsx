...
export default function CharacterList({ id }: { id: String }) {
    const { data, loading, error } = useFetchData('https://rickandmortyapi.com/api/character')
    ...
    const selectedCharacters = [dataCharacter?.character1?.characterSelected?.id, dataCharacter?.character2?.characterSelected?.id]

    return (
        loading ? 'Loading' : error ? <strong>{error.message}</strong>
            :
            <div className='grid grid-cols-2 gap-2'>
                {
                    dataCharacter?.[id]?.characterList?.map((character: ICharacter) => (
                        <div
                            key={character.id}
                            onClick={() => {
                                setDataCharacter?.(prevDataCharacter => ({ ...prevDataCharacter, [id]: { ...prevDataCharacter?.[id], characterSelected: character } }))
                            }}
                            className={`flex items-center content-center w-full rounded-xl border border-transparent ${dataCharacter?.[id]?.characterSelected?.id === character?.id ? 'border-white' : ''} ${!(dataCharacter?.[id]?.characterSelected?.id === character?.id) && selectedCharacters.includes(character.id) ? 'pointer-events-none opacity-50' : ''}`}>
